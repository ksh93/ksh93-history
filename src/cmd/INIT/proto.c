...
#line 13
static const char id[] = "\n@(#)$Id: proto (AT&T Research) 2004-10-22 $\000\n";
...
#line 349 "/home/gsf/src/lib/libpp/pplib.h"

#line 1 "../../lib/libpp/ppdef.h"
 
#line 350 "/home/gsf/src/lib/libpp/pplib.h"
...
#line 351 "/home/gsf/src/lib/libpp/pplib.h"
...
copyright __PARAM__((Notice_t* notice, register Buffer_t* b), (notice, b)) __OTORP__(Notice_t* notice; register Buffer_t* b;){
	...
	if (notice->test)
		...
		time(&clock);
	t = ctime(&clock) + 20;
	if ((x = notice->item[12].data) && sstrncmp( x, t, 4))
	{
...
astlicense __PARAM__((char* p, int size, char* file, char* options, int cc1, int cc2, int cc3), (p, size, file, options, cc1, cc2, cc3)) __OTORP__(char* p; int size; char* file; char* options; int cc1; int cc2; int cc3;){
	...
	notice.item[13] = notice.item[1] = lic[notice.type];
	notice.item[13].quote = notice.item[1].quote = 0;
	contributor = i = k = 0;
	line = 0;
	for (;;)
	{
	...
	if (!k)
		return 0;
	if (notice.type == 1 && (!notice.verbose || !notice.item[8].data))
		return 0;
	if (notice.type != 4)
	{
		if (!notice.type)
		...
		if (notice.type == 6)
		{
			copy(&tmp, notice.item[10].data ? "and": "This software", -1);
		...
		{
			copy(&tmp, notice.item[10].data ? "and it": "This software", -1);
		...
		else if (notice.type == 7)
		{
			...
			if (notice.item[i = 4].data || notice.item[i = 2].data)
				expand(&notice, &tmp, &notice.item[i]);
			else
		...
		else
		{
			...
			if (notice.type >= 14 && !notice.item[14].data)
			{
		...
		if (v = notice.item[8].data)
		{
			x = v + notice.item[8].size;
			if (*v == '\n')
				v++;
			item.quote = notice.item[8].quote;
			do
			{
	...
	if (notice.type == 4)
	{
		...
		{
			copy(&buf, "[-license?", -1);
			expand(&notice, &buf, &notice.item[14]);
			((( &buf)->nxt<( &buf)->end)?(*( &buf)->nxt++=( ']')):(( ']'),(-1)));
		}
		((( &buf)->nxt<( &buf)->end)?(*( &buf)->nxt++=( '\n')):(( '\n'),(-1)));
	}
...
#line 302 "/home/gsf/src/lib/libpp/ppproto.c"
...
#line 239 "/home/gsf/src/cmd/proto/proto.c"
