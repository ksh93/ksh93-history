...
0123)	ARGV0="-a $COMMAND"
	USAGE=$'
[-?
@(#)$Id: ditto (AT&T Labs Research) 2001-11-26 $
]
'$USAGE_LICENSE$'
...
[+?The source and destination hierarchies are generated by \btw\b(1) with
	the options \b--physical --chop\b. An \b--expr\b option may
...
integer debug=0 delete=0 exec=1 mode=1 owner=0 update=0 verbose=0

typeset local=$SHELL remote=rsh
typeset checksum='"-"'
...
if [[ $src_host ]]
then	(rsh $src_user $src_host ". ./.profile && cd $src_dir && ${tw[*]}") 2>&1 |&
else	(cd $src_dir && eval "${tw[@]}") 2>&1 |&
fi
exec 5<&p 7>&p 7>&-
if [[ $dst_host ]]
then	(rsh $dst_user $dst_host ". ./.profile && cd $dst_dir && ${tw[*]}") 2>&1 |&
...
do
	...
	then
		...
		elif [[ $src_type != d ]] && (( update && src_mtime > dst_mtime || ! update && ( src_mtime != dst_mtime  || src_size != dst_size ) ))
		then
			if [[ $src_path != . ]]
			then
				cp[ncp++]=$src_path
...
then
	...
	print
		...
		then	
			$remote $dst_user $dst_host "cd $dst_dir && $local"
		else
			( cd $dst_dir && $local )
		fi
	}
...
then
	...
	pax $paxwriteflags
		...
		if (( ! exec ))
		then
			gunzip | pax $paxreadflags
		elif [[ $dst_host ]]
		then	
			$remote $dst_user $dst_host ". ./.profile && { test -d '$dst_dir' || mkdir -p '$dst_dir' ;} && cd '$dst_dir' && gunzip | pax $paxreadflags"
		else
