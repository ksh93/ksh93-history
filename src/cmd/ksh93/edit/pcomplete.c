...
char **ed_pcomplete(struct Complete *comp, const char *line, const char *prefix, int index)
{
	...
	{
		int noglob = sh_isoption(shp,SH_NOGLOB);
		stkseek(shp->stk,0);
		if(!comp->globpat)
			wordlist = true;
		if(wordlist)
		...
		do_wordlist:
			if(!noglob)
				sh_onoption(shp,SH_NOGLOB);
		stkseek(shp->stk,0);
			sfprintf(shp->stk,"print -u %d -f '%%s\\\n' %s",sffileno(tmp),comp->wordlist);
		}
		else
		{
			if(noglob)
				sh_offoption(shp,SH_NOGLOB);
			sfprintf(shp->stk,"print -u %d -f '%%s\\\n' %s",sffileno(tmp),comp->globpat);
		}
		sfputc(shp->stk,0);
