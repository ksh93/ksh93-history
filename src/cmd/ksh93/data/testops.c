...
"[+DESCRIPTION?\btest\b evaluates expressions and indicates is "
	...
	"[+2?If first operand is \b!\b, the result is True if the second "
		...
		"as one of the unary expressions defined below.]"
	"[+3?If first operand is \b!\b, the result is True if the second "
		"and third operand evaluated as a unary expression is True.  "
	...
	"[+4?If first operand is \b!\b, the result is True if the next "
		"three operands are a valid binary expression is True.]"
"}"
...
"[+?Unary expressions can be one of the following:]{"
	...
	"[+-x \afile\a?True if \afile\a exists and is executabe.  For a "
