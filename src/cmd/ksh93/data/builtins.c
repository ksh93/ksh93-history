...
"[h?Obsolete.  Causes each command whose name has the synxax of an "
...
"[o]:?[option?If \aoption\a is not specified, the list of options and "
	"there current settings will be written to standard output.  When "
	...
	"The value of \aoption\a must be one of the following:]{"
		...
		"[+ignoreeof?Prevents an interactive shell for exiting on "
...
"[p?Privileged mode.  Disabling \b-p\b sets the effective user id to the "
	"real user id, and the effective group id to the read group id.  "
	...
	"whenever the real and effective user id is not equial or the "
...
"[-?\n@(#)break (AT&T Labs Research) 1999-04-07\n]"
USAGE_LICENSE
"[+NAME?break - break out of loop ]"
"[+DESCRIPTION?\bbreak\b is a shell special built-in the exits the "
...
"[-?\n@(#)continue (AT&T Labs Research) 1999-04-07\n]"
USAGE_LICENSE
"[+NAME?continue - continue execution at top of the loop]"
"[+DESCRIPTION?\bcontinue\b is a shell special built-in the continues " 
...
"[-?\n@(#)alias (AT&T Labs Research) 1999-07-07\n]"
USAGE_LICENSE
...
"[+DESCRIPTION?\balias\b creates or redefines alias definitions "
	...
	"If an alias value ends is a space or tab, then the word "
...
"[+?\balias\b is built-in the shell as a declaration command so that "
...
"[p?Causes the output to be in a form of alias commands that can be used "
...
"[t?Used for tracked aliases.  These are aliases that connect a "
	...
	"when the \bPATH\b variable is unset.  Tracked aliases feature are "
	"now obsolete.]"
...
"[-?\n@(#)builtin (AT&T Labs Research) 1999-07-10\n]"
USAGE_LICENSE
...
const char sh_optcd[] =
"[-?\n@(#)cd (AT&T Labs Research) 1999-06-05\n]"
USAGE_LICENSE
...
"[L?Handle each pathname component \b..\b in a logical fashion by moving "
	"up one level by name in the preset working directory.]"
...
"[-?\n@(#)command (AT&T Labs Research) 1999-04-07\n]"
USAGE_LICENSE
...
"[-?\n@(#)eval (AT&T Labs Research) 1999-07-07\n]"
USAGE_LICENSE
"[+NAME?eval - create a shell command and process it]"
"[+DESCRIPTION?\beval\b is a shell special built-int command that constructs "
...
"[-?\n@(#)exec (AT&T Labs Research) 1999-07-10\n]"
USAGE_LICENSE
...
"[+?Because \bexec\b is a special command, any failure will cause the "
	...
	"invoked \bexec\b from the \bcommand\b utility.]"
"[+?\bexec\b cannot be invoked from a restricted shell to manipulate file "
	"descriptors.]"
...
"[-?\n@(#)exit (AT&T Labs Research) 1999-07-07\n]"
USAGE_LICENSE
...
"[-?\n@(#)export (AT&T Labs Research) 1999-07-07\n]"
USAGE_LICENSE
...
"[+?\bexport\b is built-in the shell as a declaration command so that "
	"field splitting and pathname expansion are not performed on "
	"the arguments.  Tilde expansion occurs on \avalue\a.]"
"[p?Causes the output to be in a form of \bexport\b commands that can be used "
...
"[+?Each time it is invoked, the \bgetopts\b utility places the value "
  ...
  "argument, the \bgetopts\b places the option argument in the shell "
  "variable \bOPTARG\b.]"
...
"{ "
	...
	"[+--????troff?To generate a man page in \btroff\b format.]"
"}"
...
"[+?A \ajob\a can be specified as one of the following:]{"
	...
	"[+%%-?Refers to the previous job.]"
"}"
"[+?If the signal is not specified with either the \b-n\b or \b-s\b option, "
	"the \bSIGTERM\b signal is used.]"
...
"[l?List signal or signals rather than sending signals as described above.  "
...
"[+?When the \b-f\b option is specified and there are mored \astring\a "
...
"[-?\n@(#)printf (AT&T Labs Research) 1999-06-21\n]"
USAGE_LICENSE
...
"[+?In addition to the format specifier extensions, the following "
	...
	"[+-?The escape sequence \b\\E\b expands to the escape character "
		"which is octal \b033\b in ASCII.]"
...
"[-?\n@(#)pwd (AT&T Labs Research) 1999-06-07\n]"
USAGE_LICENSE
...
"[L?The absolute pathname may contains symbolic link components.  This is "
	"then default.]"
"[P?The absolute pathname will not contains any symbolic link components.]"
"[+EXIT STATUS?]{"
	"[+0?Successful completion.]"
	"[+>0?An error occurred.]"
"}"
...
"[-?\n@(#)read (AT&T Labs Research) 1999-04-07\n]"
USAGE_LICENSE
...
"[+?If there are more variables than fields, the remaining variables are "
	...
	"to the last variable.  If no \avar\a is specifed than the variable "
	"\bREPLY\b is used.]"
"[+?If you specify \b?\b\aprompt\b after the first \avar\a, then \bread\b "
...
"[-?\n@(#)readonly (AT&T Labs Research) 1999-07-07\n]"
USAGE_LICENSE
...
"[+?\breadonly\b is built-in the shell as a declaration command so that "
	"field splitting and pathname expansion are not performed on "
	"the arguments.  Tilde expansion occurs on \avalue\a.]"
...
"[-?\n@(#)return (AT&T Labs Research) 1999-07-07\n]"
USAGE_LICENSE
"[+NAME?return - return from a function or dot script ]"
"[+DESCRIPTION?\breturn\b is shell special built-in that causes the "
...
"+[-?\n@(#)set (AT&T Labs Research) 1999-09-28\n]"
USAGE_LICENSE
...
"[-?\n@(#)shift (AT&T Labs Research) 1999-07-07\n]"
USAGE_LICENSE
"[+NAME?shift - shift positional parameters]"
"[+DESCRIPTION?\bshift\b is shell special built-in that shifts the "
...
"[-?\n@(#)sleep (AT&T Labs Research) 1999-04-07\n]"
USAGE_LICENSE
...
"[-?\n@(#)trap (AT&T Labs Research) 1999-07-17\n]"
USAGE_LICENSE
"[+NAME?trap - trap signals can conditions]"
"[+DESCRIPTION?\btrap\b is a special built-in that defines actions to be "
	...
	"standard output.]"
"[+?If action is \b-\b, \btrap\b resets each \acondition\a "
	"to the default value.  If action is an empty string, the "
...
"[+?\acondition\a can be the name or number of a signal, or one of the "
	...
	"[+EXIT?This trap is executed when the shell exits.  If defined "
		...
		"word, the trap is executed in the callers environment "
...
"+[-?\n@(#)typeset (AT&T Labs Research) 1999-07-07\n]"
USAGE_LICENSE
"[+NAME?typeset - declare or display variables with attributes]"
"[+DESCRIPTION?Without the \b-f\b options, \btypeset\b sets, unsets, "
	...
	"options are unset.  If \b=\b\avalue\a is specified value is assigned "
...
"[+?When \btypeset\b is called inside a function defined with the "
	...
	"will be created.]"
"[+?Not all options combinations are possible.  For example, the numeric "
	"options \b-i\b, \b-E\b, and \b-F\b can not be specified with "
...
"[+?If no \aname\as are specified then variables that have the specified "
	...
	"specified, then the names and attributes of all variables are "
	"written to standard output.  When \b-f\b is specified, the "
	"names displayed will be function names.]"
...
"[+?\btypeset\b is built-in the shell as a declaration command so that "
	"field splitting and pathname expansion are not performed on "
	"the arguments.  Tilde expansion occurs on \avalue\a.]"
...
"[u?Without \b-f\b, convert lowercase character to uppercase and unsets "
	"\b-l\b.  With \b-f\b specifies that \aname\a is a function that "
	"hasn't bee loaded yet.]"
...
"[S?Causes the file creation mask to be written as a symbolic value rather "
	"than an octal number.]"
"\n"
"\n[mask]\n"
"\n"
"[+EXIT STATUS?]{"
...
"[-?\n@(#)unset (AT&T Labs Research) 1999-07-07\n]"
USAGE_LICENSE
...
"[-?\n@(#)unalias (AT&T Labs Research) 1999-07-07\n]"
USAGE_LICENSE
...
"[-?\n@(#)wait (AT&T Labs Research) 1999-06-17\n]"
USAGE_LICENSE
...
"\n[job ...]\n"
"\n"
"[+EXIT STATUS?If \await\a is invoked with one or more \ajob\as, all of "
...
"[+DESCRIPTION?Without \b-v\b, \bwhence\b writes on standard output an "
	...
	"not found, then no output is produced.]"
