...
static void copyto(register Mac_t *mp,int endch, int newquote)
{
	...
	while(1)
	{
		...
		switch(n)
		{
			...
			else if(mp->pattern==4 && (*cp==',' || *cp==LBRACE || *cp==RBRACE))
				break;
			...
			break;
		    case S_BRACT:
			if((mp->assign==1 || mp->arith || endch==RBRACT) &&
				 !(mp->quote || mp->lit))
			{
				int oldpat = mp->pattern;
				stakwrite(first,++c);
				first = fcseek(c);
				mp->pattern = mp->assign|mp->arith;
				copyto(mp,RBRACT,0);
				stakputc(RBRACT);
				mp->pattern = oldpat;
				cp = first = fcseek(0);
	...
	else if(mp->pattern>=2 || (mp->pattern && nopat))
	{
		...
		while(size-->0)
		{
			...
			if(nopat&&(c==S_PAT||c==S_ESC||c==S_BRACT||c==S_ENDCH))
				...
				c=1;
			else if(mp->pattern==3 && c==S_ESC && state[*(unsigned char*)cp]==S_DIG)
				c=1;
			else
				c=0;
			if(c)
