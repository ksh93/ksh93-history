...
"[-1c?\n@(#)$Id: type (AT&T Labs Research) 2006-04-18 $\n]"
USAGE_LICENSE
...
#endif
	dp->nodes = (char*)(dp+1);
	dp->data = dp->nodes + pp->numnodes*NV_MINSZ;
...
static Namval_t *create_type(Namval_t *np,const char *name,int flag,Namfun_t *fp)
{
	...
	if(dp->numnodes && dp->strsize<0)
	{
		...
		{
			if(nq->nvname[m]=='.' && memcmp(name,&nq->nvname[m+1],n+1)==0) 
				goto found;
		}
		nq = 0;
	}
...
static int typeinfo(Opt_t* op, Sfio_t *out, const char *str, Optdisc_t *fp)
{
	...
	if(dp->ndisc>0)
	{
		...
		stakputc('.');
		n = staktell();
		sfprintf(out,"[+?\b%s\b defines the following discipline methods:]{\n",np->nvname);
...
{
	Namval_t	*mp=nodes[0], *np, *nq, **mnodes=nodes;
	...
	for(nnodes=1,i=1; i <numnodes; i++)
	{
		...
		if(dp = (Namtype_t*)nv_hasdisc(np, &type_disc))
		{
			...
			if((n=dp->strsize)<0)
				n = -n;
			iref = nref = dp->nref;
			if(np->nvname[m]=='_' && np->nvname[m+1]==0 && nv_type(np))
			{
				qp = dp;
				nd = dp->ndisc;
				nnodes = dp->numnodes;
			...
			n = strlen(np->nvname);
			while(nodes[i+1] && (cp=nodes[i+1]->nvname) && memcmp(cp,np->nvname,n)==0 && cp[n]=='.')
				i++;
		}
	...
	if(nd)
		nd++;
	pp = newof(NiL, Namtype_t, 1, nnodes*NV_MINSZ + offset + size + (nnodes+nd)*sizeof(char*) + iref*sizeof(struct Namref));
	pp->fun.dsize = sizeof(Namtype_t)+nnodes*NV_MINSZ +offset;
	pp->fun.type = mp;
	...
	for(name=0, offset=0, k=i=1; i < numnodes; i++)
	{
		...
		*cp++ = 0;
		...
		if(dp = (Namtype_t*)nv_hasdisc(np, &type_disc))
		{
			...
			while(nodes[i+1] && (cname=&nodes[i+1]->nvname[m]) && memcmp(cname,&np->nvname[m],n)==0 && cname[n]=='.')
			{
				int j=kfirst;
				nv_unset(np);
				dtdelete(root,np);
